...
final class AfdianOrderRepository extends Repository
{
    ...
    public function createIfAbsent(array $data): array
    {
        ...
        $stmt = $this->pdo()->prepare('INSERT INTO afdian_orders (out_trade_no, trade_no, user_id, buyer_name, remark, plan_id, sku_detail, amount_cents, status, raw_payload, voicehub_status, voicehub_attempts, voicehub_last_error, created_at, paid_at, processed_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NULL, ?, ?, NULL, ?)');
        try {
            $stmt->execute([
                $data['out_trade_no'],
                $data['trade_no'] ?? '',
    ...
    {
        $allowed = ['trade_no', 'user_id', 'buyer_name', 'remark', 'plan_id', 'sku_detail', 'amount_cents', 'status', 'raw_payload', 'voicehub_status', 'voicehub_attempts', 'voicehub_last_error', 'paid_at', 'created_at', 'processed_at'];
        $sets = [];
        $params = [];
